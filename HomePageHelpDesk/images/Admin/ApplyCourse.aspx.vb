...
Imports System.Net
Partial Class Admin_ApplyCourse
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                MyCLS.ConOpen()
                mailid = Request.QueryString("email")
                stid = Request.QueryString("stid")
                ViewState("stid") = Request.QueryString("stid")
                FillDDl()
                filldata()
    ...
    Private Sub FillDDl()
        Try
            Dim strq1 As String = "SELECT c.courseid,c.coursecode,c.CourseTitle,CONVERT(varchar, c.cStartDate, 101) as cStartDate, CONVERT(varchar, c.cEndDate, 103) as cEndDate FROM COURSE C " & _
                                  " WHERE c.Courseid not in(select courseid from studentregcourse where stid=" & stid & ")"
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If (ddlCourse.SelectedIndex = 0) Then
            ScriptManager.RegisterStartupScript(Me, GetType(Page), UniqueID, "alert('Please select course !');", True)
            Return
        End If
        Dim dsAppSt As New DataSet
        dsAppSt = CLS.fnQuerySelectDs("SELECT aid,email from student where stid =" & ViewState("stid"))
        If dsAppSt.Tables IsNot Nothing Then
            If dsAppSt.Tables(0).Rows IsNot Nothing Then
                If dsAppSt.Tables(0).Rows.Count > 0 Then
        ...
        dsAnotherCourse = CLS.fnQuerySelectDs("select top 1 1 from StudentAppCourse where aid =" & ViewState("aid") & " and courseid=" & ddlCourse.SelectedValue & "")
            ...
            If dsAnotherCourse.Tables(0).Rows IsNot Nothing Then
                ...
                Else
                    ...
                    If retVal > 0 Then
                        ...
                        Dim strq As String = "select CourseTitle,screeningExam from course where CourseID=" & ddlCourse.SelectedValue
                        ...
                        Else
                            ScriptManager.RegisterStartupScript(Me, GetType(Page), UniqueID, "alert('Course applied successfully !');; location.href='ManageStudentInfo.aspx' ;", True)
                        End If
                    End If
                End If
            End If
