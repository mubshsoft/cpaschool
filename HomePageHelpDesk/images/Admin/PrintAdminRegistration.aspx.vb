...
Partial Class Admin_PrintAdminRegistration
    ...
    Sub filldata()
        Try
            ...
            Dim mailid As String = Session("PrintUser")


            'Dim strCourse As String
            'Dim dsCourse As New DataSet
            'strCourse = "select courseid,coursetitle from course where courseid=" & Session("cid")
            ...
            'dsAppSt = CLS.fnQuerySelectDs("select * from application where email = '" & mailid & "' and c")

            Dim strapp As String
            strapp = "select stap.courseid,cr.coursetitle as coursetitle ,ap.* from studentappcourse stap inner join course cr on stap.courseid=cr.courseid inner join application ap on ap.aid=stap.aid where ap.email = '" & mailid & "' and stap.courseid=" & Session("cid")
            dsAppSt = CLS.fnQuerySelectDs(strapp)
                ...
                If dsAppSt.Tables IsNot Nothing Then
                    If dsAppSt.Tables(0).Rows IsNot Nothing Then
                        If dsAppSt.Tables(0).Rows.Count > 0 Then
                            ...
                            lblCourseID.Text = dsAppSt.Tables(0).Rows(0)("coursetitle").ToString
