...
Partial Class Admin_ListFaculty
    ...
    Sub bindgrid()
        Try
            Dim strq As String = "select ROW_NUMBER() OVER(ORDER BY fid asc) AS ROWID,Fid,email,firstName,lastname,(firstName + ' ' + lastname) as FullName from FacultyRegistration where status=0"
    ...
    Protected Sub GrdFaculty_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GrdFaculty.RowCommand
        ...
        Try
            If e.CommandName = "Delete" Then
                ...
                Dim str As String = "update FacultyRegistration set status=1 where FId=" & id
                ExeNQcomsp(str)
                bindgrid()
                Response.Redirect("ListFaculty.aspx")
                'ScriptManager.RegisterStartupScript(Me, GetType(Page), UniqueID, "alert('Faculty Deleted successfully!'); location.href='ListFaculty.aspx' ;", True)
