...
Partial Class Student_ListCourseForfee
    ...
    Sub bindgrid()
        Try
            Dim strq As String = "select st.stid,sr.courseid,c.CourseCode,c.Coursetitle,st.email,st.firstName,st.lastname,feestatus=isnull((select top(1) convert(int,FeeStatus) from StudentSemester where CourseId=sr.courseid and stid=sr.stid),1) from student st INNER JOIN StudentRegcourse sr on st.stid=sr.stid INNER JOIN course c on c.CourseID=sr.courseid where st.email ='" & Session("username") & "'"
