...
Imports fmsf.DAL
Imports fmsf.lib
Partial Class Student_ListvideoConfrence
    ...
    Private Sub FillCourse()
        Try
            ...
            Dim strBatch As String = " select c.* from StudentRegcourse sr INNER JOIN course c on c.CourseID=sr.courseid inner join student s on sr.stid=s.stid where  s.email ='" & Session("username").ToString() & "'"
            Dim dsbatch As New DataSet()
            dsbatch = CLS.fnQuerySelectDs(strBatch)
            ddlCourse.DataSource = dsbatch
    ...
    Sub bindgrid1()
        Try
            ...
            End If
            Dim strq As String = "SP_GetVideoconfrencedetails_student " & CID & "," & BID & "," & SID & ",'" & fromdt & "','" & todt & "'," & Session("stid")
